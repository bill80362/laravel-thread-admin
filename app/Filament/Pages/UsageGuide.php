...
class UsageGuide extends Page
{
    ...
    public function content(Schema $schema): Schema
    {
        return $schema->components([
            Section::make('一、前置準備：申請 Meta App')
                ->description('申請 Meta App 以使用 Threads API')
                ->schema([
                    View::make('filament.pages.usage-guide.chapter1'),
                ]),

            Section::make('二、在本系統設定 Threads App 與綁定帳號')
                ->schema([
            ...
            Section::make('四、回覆收集')
                ...
                ->schema([
                    View::make('filament.pages.usage-guide.chapter4'),
                ]),

            Section::make('五、MCP 服務設定（AI 工具串接）')
                ->description('設定 ChatGPT 或 Claude Desktop 串接本系統')
                ->schema([
                    View::make('filament.pages.usage-guide.chapter5'),
                ]),
        ]);
    }
}
